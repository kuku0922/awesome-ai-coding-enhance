...
您是专门从事企业应用程序的Vue 3开发者。使用带有TypeScript的组合式API，实现Pinia进行状态管理，确保WCAG 2.1 AA可访问性。

## 工作流程
1. 分析需求并设计架构
2. 使用Vue 3 + TypeScript + Vite设置项目
3. 构建具有适当类型和可访问性的组件
4. 使用Pinia实现状态管理
5. 使用Vitest和Playwright添加测试
6. 优化性能并部署
...
您是专门从事企业门户应用的Vue 3专家，具有深厚的TypeScript知识和企业架构模式。

## 专业知识
- 具有高级模式的Vue 3组合式API
- 类型安全的TypeScript集成
- Pinia状态管理和Vue Router
- 企业测试策略（Vitest、Playwright）
- 性能优化和可访问性
- 组件库和设计系统
- 企业安全和集成模式
...
提供具有全面测试和文档的企业级Vue 3解决方案。
```

### 变体3：全面版（~12K tokens）- 当前版本
这是具有所有高级功能、测试场景和全面覆盖的完整详细提示词。用于需要最大细节和指导的复杂企业项目。

**建议**：从当前全面版开始，获得最大的企业级指导。
- 使用简洁版进行快速原型设计或在token限制受限时使用
- 使用平衡版进行标准企业开发项目
- 使用全面版进行复杂的、关键任务的企业应用程序

## 优化技巧

### Token优化
- **当前token数量**：~11,800 tokens
- **优化机会**：
  1. **示例部分**（~2,500 tokens）：可以浓缩为更短的模式
  2. **测试场景**（~2,000 tokens）：可以总结为常见模式
  3. **最佳实践部分**（~1,500 tokens）：可以专注于最关键的实践
  4. **工作流程描述**（~1,200 tokens）：可以使用更简洁的要点
  5. **专业知识部分**（~1,000 tokens）：可以删除冗余描述

### 清晰度改进
- **潜在模糊之处**：
  1. "企业模式" → "特定的微前端和设计系统模式"
  2. "全面测试" → "单元测试（>90%）、集成测试、关键路径的端到端测试"
  3. "性能优化" → "包大小<500KB、Lighthouse >90、初始加载<3秒"
  4. "安全最佳实践" → "OWASP合规、依赖扫描、输入验证"

### 效果增强
- **考虑添加**：
  1. **特定代码模式库**：常见企业组件模式
  2. **迁移指南**：Vue 2到Vue 3企业迁移策略
  3. **集成示例**：特定企业系统集成模式
  4. **故障排除指南**：常见企业Vue.js问题和解决方案
  5. **性能预算模板**：特定性能目标和监控设置

### 迭代指南
在企业环境中测试此提示词后：
1. **跟踪响应质量**：注意哪些部分提供最有价值的信息
2. **识别差距**：寻找缺失的企业特定模式或需求
3. **衡量效果**：评估指导是否导致生产就绪的代码
4. **基于反馈优化**：根据团队需求调整复杂度和重点
5. **版本控制**：为不同项目规模维护不同版本
6. **团队培训**：使用提示词的部分进行团队入职和标准制定
